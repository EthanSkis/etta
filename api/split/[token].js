// ettacalls.com/split/<token> — a sibling's share of the bill.
//
// The only page here meant for someone who isn't the account holder: they
// were sent this link by the family member who pays. Forwards POSTs, since
// starting or stopping a share happens on this page.

const SHARE_ENDPOINT =
  "https://kkqgxojxsfqgfpzdyzjv.supabase.co/functions/v1/share";
...
export default async function handler(req, res) {
  ...
  try {
    const upstream = await fetch(`${SHARE_ENDPOINT}/${token}`, {
    ...
    console.error("share proxy failed:", err);
    res.setHeader("Content-Type", "text/html; charset=utf-8");
    return res.status(502).send(
      "<!DOCTYPE html><meta charset=utf-8><title>Etta</title>" +
        "<p style=\"font:16px/1.5 system-ui;padding:24px\">This page is briefly " +
        "unavailable, and nothing was charged. Please try again in a moment.",
    );
  }
}
